...
- 基本可用 （Basically Available）
  ...
  相对于CAP理论中可用性的要求：【任何时候，读写都是成功的】，“基本可用”要求系统能够基本运行，一直提供服务，强调的是分布式系统在出现不可预知故障的时候，允许损失部分可用性。比如系统通过断路保护而引发快速失败，在快速失败模式下，支持加载默认显示的内容（静态化的或者被缓存的数据），从而保证服务依然可用。
...
```bash
root@prometheus-221:~ 16:03:11 # cat /sys/kernel/mm/transparent_hugepage/enabled
always [madvise] never
root@prometheus-221:~ 16:04:30 #
...
OK
127.0.0.1:6379> config get slowlog-max-len  slowlog-log-slower-than 
...
# 查看慢日志的最近n条记录，默认为 10
# 第二行表示命令执行的时间戳，距离 1970-1-1 的秒数，date -d +@1544690617 可以转换
# 第三行表示每条指令的执行时长 单位 微秒
127.0.0.1:6379> slowlog get 1
1) 1) (integer) 3
   2) (integer) 1760609047
   3) (integer) 5
   4) 1) "slowlog"
      2) "len"
   5) "127.0.0.1:57336"
